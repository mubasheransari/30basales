...
import 'package:new_amst_flutter/Widgets/toast_widget.dart';
...
checkmarkColor: Colors.white,
                  //  iconTheme: IconThemeData(color:selected ? Colors.white : kText,applyTextScaling: true ),
                    label: Text(label, style: const TextStyle(fontFamily: 'ClashGrotesk',fontSize: 13)),
                    selected: selected,
                    onSelected: (_) => setState(() => _selectedBrand = label),
                    selectedColor: Colors.grey[400],//const Color(0xFF7F53FD),
...
class _CartScreenSkuOnlyState extends State<_CartScreenSkuOnly> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (!mounted) return;
      toastWidget("Sales Added!", Colors.green);
      // ScaffoldMessenger.of(context).showSnackBar(
      //   const SnackBar(content: Text('Sale Successfully dded')),
      // );
      Navigator.pop(context, {'submitted': true});
    } catch (e) {
      if (!mounted) return;
       toastWidget("Error saving sale: $e!", Colors.red);
      // ScaffoldMessenger.of(context).showSnackBar(
