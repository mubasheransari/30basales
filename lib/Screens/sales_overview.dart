...
class SalesChartSection extends StatelessWidget {
  ...
  // Keep same project theme gradient
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<OrderRecord>>(
      ...
      builder: (context, snap) {
        ...
        if (data.isEmpty) {
          return _ModernEmptyCard(scale: s);
        }
        ...
        double changePct = 0;
        if (prev7Total > 0) changePct = ((last7Total - prev7Total) / prev7Total) * 100;
        ...
        // Bottom label interval: avoid crowding for many days
        final int labelEvery = sortedDays.length <= 7
            ? 1
            : (sortedDays.length <= 14 ? 2 : (sortedDays.length <= 30 ? 4 : 6));
                ...
                children: [
                  ...
                  Expanded(
                    child: ClipRRect(
                      ...
                      child: Container(
                        padding: EdgeInsets.fromLTRB(10 * s, 10 * s, 10 * s, 6 * s),
                        decoration: BoxDecoration(
                          ...
                          LineChartData(
                            ...
                            lineTouchData: LineTouchData(
                              ...
                              touchTooltipData: LineTouchTooltipData(
                                tooltipRoundedRadius: 14 * s,
                                tooltipPadding: EdgeInsets.all(10 * s),
                                tooltipMargin: 10 * s,
                              ...
                              bottomTitles: AxisTitles(
                                sideTitles: SideTitles(
                                  ...
                                  getTitlesWidget: (value, meta) {
                                    ...
                                    if (i % labelEvery != 0 && i != sortedDays.length - 1) {
                            ...
                            lineBarsData: [
                              LineChartBarData(
                                ...
                                dotData: FlDotData(
                                  show: points.length <= 14, // ✅ cleaner for big datasets
                                ),
                                // keep it clean (no hard-coded single color look)
                                gradient: _kGrad,
                                belowBarData: BarAreaData(
                                  ...
                                  gradient: LinearGradient(
                                    ...
                                    colors: [
                                      const Color(0xFF7F53FD).withOpacity(0.18),
                  ...
                  Row(
                    ...
                    children: [
                      Row(
                        children: [
                          Icon(Icons.touch_app_rounded, size: 14 * s, color: const Color(0xFF64748B)),
                          SizedBox(width: 5 * s),
                          Text(
                      ...
                      Row(
                        children: [
                          Container(
                            ...
                            child: ShaderMask(
                              shaderCallback: (r) => _kGrad.createShader(r),
...
class _GlassCard extends StatelessWidget {
  const _GlassCard({required this.child, required this.scale});
  final Widget child;
  final double scale;
...
class _ModernEmptyCard extends StatelessWidget {
  const _ModernEmptyCard({required this.scale});
  final double scale;
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: Icon(Icons.insights_rounded, color: Colors.white, size: 26 * scale),
              ),
              ...
              Text(
                'Add some orders to see your insights here.',
...
class _GradIconCircle extends StatelessWidget {
  const _GradIconCircle({required this.icon, required this.scale});
  final IconData icon;
  final double scale;
...
class _HeaderPill extends StatelessWidget {
  const _HeaderPill({required this.text, required this.dotColor, required this.scale});
  final String text;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Container(width: 8 * scale, height: 8 * scale, decoration: BoxDecoration(shape: BoxShape.circle, color: dotColor)),
...
class _StatChipModern extends StatelessWidget {
  const _StatChipModern({
    required this.label,
    required this.value,
    required this.dot,
    required this.scale,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Container(width: 8 * scale, height: 8 * scale, decoration: BoxDecoration(shape: BoxShape.circle, color: dot)),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(up ? Icons.trending_up_rounded : Icons.trending_down_rounded, size: 16 * scale, color: color),
          SizedBox(width: 6 * scale),
          Text(
