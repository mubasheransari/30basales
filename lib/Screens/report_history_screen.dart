...
class _ReportHistoryScreenState extends State<ReportHistoryScreen> {
  ...
  List<DailySheet> get _filtered {
    ...
    switch (_filter) {
      ...
      case _Filter.today:
        return _sheets.where((s) => _dateOnly(s.day) == today).toList();

      case _Filter.last7:
        final from = now.subtract(const Duration(days: 7));
        final fromDateOnly = _dateOnly(from);
        return _sheets
            .where((s) => _dateOnly(s.day)
                .isAfter(fromDateOnly.subtract(const Duration(days: 1))))
            .toList();

      case _Filter.thisMonth:
        return _sheets
            .where((s) => s.day.year == now.year && s.day.month == now.month)
            .toList();
    }
  }
  ...
  // ✅ UPDATED: include weight in summary line
  String _daySummary(DailySheet sheet) {
    final w = sheet.totalWeight;
    return '${sheet.totalLines} SKUs • ${sheet.totalQty} Qty • ${sheet.totalOrders} orders • ${w.toStringAsFixed(2)} KG';
  }

  // ✅ NEW: separate weight row text
  String _weightOnly(DailySheet sheet) {
    return '${sheet.totalWeight.toStringAsFixed(2)} KG';
  }
  ...
  Future<void> _downloadDailyPdf(DailySheet sheet) async {
    final Uint8List bytes = await _pdfService.generateDailyReportPdf(sheet.day);
      ...
      body: SafeArea(
        child: RefreshIndicator(
          ...
          child: ListView(
            physics: const AlwaysScrollableScrollPhysics(),
            padding:
                EdgeInsets.fromLTRB(16 * s, 8 * s, 16 * s, 140 * s + padBottom),
            children: [
              _FiltersBar(
                s: s,
                active: _filter,
                ...
                ..._filtered.map((sheet) {
                  ...
                  return Padding(
                    ...
                    child: _ReportCard(
                      s: s,
                      dateText: _prettyDay(sheet.day),
                      // ✅ Summary now includes weight
                      vehicleText: _daySummary(sheet),
                      // ✅ NEW: separate weight row (like separate row in card)
                      weightText: _weightOnly(sheet),
                      completed: true,
                      downloaded: downloaded,
                      onDownload: () async {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Container(
            width: 9 * s,
            height: 138 * s, // ✅ slightly taller to fit new row
            decoration: const BoxDecoration(
              borderRadius: BorderRadius.only(
                topLeft: Radius.circular(12),
                bottomLeft: Radius.circular(12),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // ✅ Row 1: SKUs / Qty / Orders / Weight summary
                  Row(
                    children: [
                      Flexible(
                        child: Text(
                  ...
                  // ✅ Row 2: Total Weight badge row (like SKU row)
                  Row(
                    children: [
                      Icon(
                        Icons.scale_rounded,
