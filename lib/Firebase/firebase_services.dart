...
class FbUserProfile {
  ...
  /// Combined display name (Mart - City - Area)
  final String? locationName;
  ...
  static FbUserProfile fromDoc(String uid, Map<String, dynamic> d) {
    ...
    if (gp is GeoPoint) {
      ...
      final latAny = gp['lat'] ?? gp['latitude'];
      final lngAny = gp['lng'] ?? gp['longitude'];
    ...
    return FbUserProfile(
      ...
      locationMartName: d['locationMartName']?.toString(),
      locationCityName: d['locationCityName']?.toString(),
      locationAreaName: d['locationAreaName']?.toString(),
      allowedLat: lat,
      allowedLng: lng,
      allowedRadiusMeters: (d['allowedRadiusMeters'] as num?)?.toDouble() ?? 100,
    );
...
class FbLocation {
  ...
  /// Combined display name (used for ordering + UI fallback)
  final String name;
  ...
  static FbLocation fromDoc(String id, Map<String, dynamic> d) {
    ...
    // New fields (fallback to old `name`)
    final mart = (d['martName'] ?? '').toString().trim();
    final city = (d['cityName'] ?? '').toString().trim();
    final area = (d['areaName'] ?? '').toString().trim();

    // Legacy fallback
    final legacyName = (d['name'] ?? d['title'] ?? id).toString().trim();

    // Build display name: "Mart - City - Area"
    final parts = <String>[
      if (mart.isNotEmpty) mart,
      if (city.isNotEmpty) city,
      if (area.isNotEmpty) area,
    ];
    final displayName = parts.isNotEmpty ? parts.join(' - ') : legacyName;

    // Allowed location primary
    double lat = 0.0;
    double lng = 0.0;
    ...
    return FbLocation(
      id: id,
      martName: mart.isNotEmpty ? mart : legacyName,
      cityName: city,
      areaName: area,
      name: displayName,
      lat: lat,
...
class FbLocationRepo {
  ...
  /// Fetches locations once and stores them in memory.
  /// Safe to call multiple times. Uses TTL.
  static Future<List<FbLocation>> warmCache({
  ...
  /// ✅ IMPORTANT FIX:
  /// Do NOT depend on orderBy('name') because some docs may temporarily miss it.
  /// Stream all docs; sort on UI / via watchLocations().
  static Stream<QuerySnapshot<Map<String, dynamic>>> streamLocations() {
    return _col.snapshots();
  }

  /// Sorted list stream (recommended for dropdowns/lists)
  static Stream<List<FbLocation>> watchLocations() {
  ...
  /// ✅ FIXED upsert:
  /// - When editing: pass same `id` to update same doc (no duplicate)
  /// - Always writes `name` (combined) for compatibility + ordering
  /// - Writes mart/city/area as separate fields
  static Future<void> upsertLocation({
    String? id,
    required String martName,
    required String cityName,
    ...
    final combined = parts.isNotEmpty ? parts.join(' - ') : (m.isNotEmpty ? m : c);
...
class FbSupervisorRepo {
  ...
  static Future<void> createSupervisor({
    ...
    final primaryApp = Firebase.app();
    const secondaryName = 'secondary_auth_app';
    ...
    if (newUid == null) throw Exception('Could not create supervisor user');
  ...
  static Future<void> deleteSupervisor(String uid) async {
    // deletes Firestore profile only (Auth deletion needs Admin SDK)
    await _col.doc(uid).delete();
  }
}
...
class FbJourneyRepo {
  ...
  static Future<FbJourneyPlan?> fetchActivePlanForToday({
    ...
    for (final d in q.docs) {
      final plan = FbJourneyPlan.fromDoc(d.id, d.data());
      if (_inRange(today, plan.startDate, plan.endDate)) return plan;
    }
    return null;
  }
  ...
  static Stream<Map<String, Map<String, dynamic>>> watchTodayVisitsMap(
    String planId,
  ) {
    final key = dateKey(DateTime.now());
    return _visits(planId).where('dateKey', isEqualTo: key).snapshots().map((q) {
