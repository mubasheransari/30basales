...
class _JourneyPlansManagementTabState extends State<JourneyPlansManagementTab> {
  ...
  child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
    ...
    builder: (ctx, snap) {
      ...
      // ✅ NO-INDEX filtering (same as your code)
      final docs = allDocs.where((d) {
        final m = d.data();
        return (m['supervisorId'] ?? '').toString() == supId;
      }).toList(growable: false);
      ...
      for (final doc in docs) {
        ...
        (byDay[dayKey] ??= []).add(doc);
      }

      // ✅ Sort day keys (latest first if yyyy-mm-dd format)
      final dayKeys = byDay.keys.toList()
        ..sort((a, b) => b.compareTo(a));

      // ✅ Helper: format "Day: yyyy-mm-dd" -> "05 Jan 2026"
      String _prettyDay(String dayKey) {
        try {
          final dt = DateTime.parse(dayKey); // expects yyyy-mm-dd
      ...
      return ListView.separated(
        ...
        itemBuilder: (_, di) {
          final dayKey = dayKeys[di];
          final dayDocs = byDay[dayKey] ?? const [];
          ...
          for (final v in dayDocs) {
            final d = v.data();
            final checkIn = d['checkIn'] as Timestamp?;
            final checkOut = d['checkOut'] as Timestamp?;
            totalDuration += _durationMin(checkIn, checkOut);
            ...
            if (displayScore != null) {
              totalDisplay += displayScore;
              displayCount++;
            }
          }

          final avgDisplay =
              displayCount > 0 ? (totalDisplay / displayCount) : null;
          ...
          return Container(
            ...
            child: Theme(
              ...
              child: ExpansionTile(
                tilePadding:
                    EdgeInsets.symmetric(horizontal: 12 * s, vertical: 2 * s),
                childrenPadding:
                    EdgeInsets.fromLTRB(12 * s, 0, 12 * s, 12 * s),
                ...
                // ✅ Day summary row
                subtitle: Text(
                  'Visits: ${dayDocs.length}'
                  '${totalDuration > 0 ? ' • Total: $totalDuration min' : ''}'
                  '${avgDisplay != null ? ' • Avg Display: ${avgDisplay.toStringAsFixed(1)}/10' : ''}'
                  '${stockCount > 0 ? ' • Stock Sum: $totalStock' : ''}',
                  maxLines: 2,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(
                    fontFamily: 'ClashGrotesk',
                ...
                // ✅ Inside: list all stops for this date
                children: [
                  ...List.generate(dayDocs.length, (i) {
                    final d = dayDocs[i].data();
                    ...
                    final shelfCondition =
                        (form['shelfCondition'] ?? '').toString();
                    final stockQty = _asInt(form['stockQty']);
                    final displayScore = _asInt(form['displayScore']);
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'In: ${_fmtTs(checkIn)} • Out: ${_fmtTs(checkOut)}'
                            ...
                            Wrap(
                              ...
                              children: [
                                ...
                                if (stockQty != null)
                                  _visitChip('Stock Qty: $stockQty'),
                                if (displayScore != null)
                                  _visitChip('Display: $displayScore/10'),
                              ],
                            ),
                          ],
